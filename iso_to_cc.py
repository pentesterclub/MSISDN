...
Used to bridge the bundled MCC/MNC dataset (which uses ISO codes) with
the MSISDN parsing (which uses E.164 CCs).

Curated subset of the most common countries. For full coverage, the
libphonenumber library is used as a runtime fallback.
"""

from __future__ import annotations

# ISO alpha-2 -> E.164 CC
ISO_TO_CC: dict[str, int] = {
    "AF": 93,   "AX": 358, "AL": 355, "DZ": 213, "AS": 1684,
    "AD": 376, "AO": 244, "AI": 1264,"AQ": 672, "AG": 1268,
    "AR": 54,  "AM": 374, "AW": 297, "AU": 61,  "AT": 43,
    "AZ": 994, "BS": 1242,"BH": 973, "BD": 880, "BB": 1246,
    "BY": 375, "BE": 32,  "BZ": 501, "BJ": 229, "BM": 1441,
    "BT": 975, "BO": 591, "BA": 387, "BW": 267, "BR": 55,
    "IO": 246, "VG": 1284,"BN": 673, "BG": 359, "BF": 226,
    "BI": 257, "KH": 855, "CM": 237, "CA": 1,   "IC": 34,
    ...
    "ET": 251, "FO": 298, "FK": 500, "FJ": 679, "FI": 358,
    ...
    "IL": 972, "IT": 39,  "JM": 1876,"JP": 81,  "JE": 44,
    ...
    "PE": 51,  "PH": 63,  "PN": 870, "PL": 48,  "PT": 351,
    ...
    "VA": 379, "VE": 58,  "VN": 84,  "WF": 681, "EH": 212,
    "YE": 967, "ZM": 260, "ZW": 263,

    # Special / non-ISO codes used in the MCC-MNC dataset
    "GE-AB": 995,  # Abkhazia (uses +995 7xx, sometimes +7 840/940)
    "XK":    383,  # Kosovo
}


def get(iso_code: str) -> int | None:
    """Return the E.164 country calling code for an ISO 3166-1 alpha-2 code."""
    if not iso_code:
        return None
    return ISO_TO_CC.get(iso_code.upper())


def all_pairs() -> list[tuple[str, int]]:
    """Return all (ISO, CC) pairs in the table."""
    return list(ISO_TO_CC.items())
